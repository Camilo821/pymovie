...
tittle = ttk.Label(window, text="Bienvenido a Pymovie", font=("Arial", 14))
...
search = ttk.Entry(window, width=25)
search.grid(column=1, row=1, padx=10, pady=5)
text_label = tk.StringVar()
text_label.set("")
results = ttk.Label(window, textvariable=text_label, font=("Arial", 8))
...
def search_movie(selection_entry, movies, movie):
    ...
    rate_label = ttk.Label(movie_window, text=f"Al {movie.rate}% de las personas les gustó")
    ...
    image_label = ttk.Label(movie_window, image=image_tk)
    image_label.image = image_tk
    image_label.grid(column=0, row=0, padx=10, pady=5)
    sinopsis_label = tk.Text(movie_window, wrap="word", height=8, width=40)  # wrap="word" asegura que el texto no corte palabras
    sinopsis_label.insert(
    "1.0",
    movie.es_sinopsis,
)
    sinopsis_label.config(state="disabled")
...
def search_info():
    ...
    selection_entry = ttk.Entry(window, width=25)
    selection_entry.grid(column=0, row=4, padx=10, pady=5)
    selection_button = ttk.Button(window, text="Buscar", command=lambda : search_movie(selection_entry, movie.movies, movie))
    selection_button.grid(column=1, row=4, columnspan=2, pady=10)


search_button = ttk.Button(window, text="Enviar", command=search_info)
